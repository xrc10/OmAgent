...
from agent.walk_to_traffic_light.walk_to_traffic_light import WalkToTrafficLight
...
workflow = ConductorWorkflow(name='navigate_traffic_light_zh')
...
# 3. Input traffic light location
task3 = simple_task(task_def_name='FindTrafficLightInput', task_reference_name='find_traffic_light_input2')

# 4. If object exists, show its location from stm
task4 = simple_task(task_def_name='WalkToTrafficLight', task_reference_name='walk_to_traffic_light')

# Create loop that continues until object is found
# Loop terminates when object is detected or user wants to stop
find_traffic_light_loop = DoWhileTask(
...
# Create loop that continues while the people is walking towards the traffic light
walk_to_traffic_light_loop = DoWhileTask(
    task_ref_name='walk_to_traffic_light_loop', 
    tasks=[task3, task4], 
    termination_condition='if ($.walk_to_traffic_light["stop_search"] == true) {false;} else {true;}'
)

# Define workflow sequence: find object loop -> exit
workflow >> find_traffic_light_loop >> walk_to_traffic_light_loop
