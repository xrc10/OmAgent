...
class DetectTrafficLightColor(BaseLLMBackend, BaseWorker):
    ...
    def _run(self, *args, **kwargs):
        ...
        if isinstance(content, str):
            content = {"decision": "wait", "reason": "返回的不是有效的JSON。"}
        ...
        if content.get("decision") == "pass":
            ...
            self.callback.send_answer(
                ...
                msg=f"你可以通过了。交通信号灯在 {traffic_light_location}。"
            )
            ...
            self.callback.send_answer(
                agent_id=self.workflow_instance_id,
                msg=f"请等待。{content.get('reason', '交通信号灯未找到，继续搜索...')}"
            )

            return {
                "decision": "wait",
                "stop_search": False,
                "reason": content.get("reason", "交通信号灯未找到，继续搜索...")
            }
